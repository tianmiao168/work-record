...
- session 机制（有状态的，）需要配合 cookie 才能实现，但是 cookie 不支持跨域，所以当涉及到跨域时需要使用 JWT（JOSN WEB TOKEN）认证机制。
- JWT 工作原理(无状态的)：用户信息通过 token 存储在浏览器的 local storage 中，服务端通过还原 token 来进行身份认证。
