node 学习网站 https://juejin.cn/post/6961101653709684772 https://github.com/qufei1993/Nodejs-Roadmap/blob/master/README.md

https://brucecai55520.gitee.io/bruceblog/notes/nodejs/express.html
浏览器就是一个运行环境，node 也是一个运行环境。

- 浏览器运行环境：浏览器运行环境中内置的 api+chrome 浏览器的 v8js 解析引擎 保证 js 代码执行

node 的应用(https://zhuanlan.zhihu.com/p/121055042)：前端工具链（Babel，webpack,eslint,jest），npm,工程化相关，前后端分离，服务端
...
## cors 中间件解决跨域（通过响应头来决定浏览器是否阻止跨域获取资源）
...
## 身份认证（session 机制，JWT 机制）

- session 机制（有状态的，会在服务端存储）需要配合 cookie 才能实现，但是 cookie 不支持跨域，所以当涉及到跨域时需要使用 JWT（JOSN WEB TOKEN）认证机制。
- JWT 工作原理(无状态的，不会在服务端存储)：用户信息通过 token 存储在浏览器的 local storage 中，服务端通过还原 token 来进行身份认证。
  客户端进行登录，服务端校验账号和密码，校验通过后将身份信息加密生成 token，返回给客户端，浏览器将 token 存储在 local storage 或 sessionStorage 中，当再次向服务端发起请求的时候将 token 赋给请求头的 Authoritarian 属性，服务端将 token 还原成身份信息进行校验，校验返回响应信息。
